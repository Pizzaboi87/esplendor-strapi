...
export const CartButton = ({
  ...
  return (
    ...
    >
      {quantity > 0 ? (
        <div className="md:w-1/2 w-full rounded-md flex items-center justify-between gap-x-[0.1rem] animate__animated animate__pulse">
          <button
            onClick={onRemove}
            disabled={disabled}
        ...
        <button
          onClick={onAdd}
          disabled={disabled}
          className="md:w-1/2 w-full uppercase bg-black text-white py-[0.625rem] px-4 rounded-md hover:bg-black/80 focus:outline-none disabled:opacity-50 transition-colors disabled:cursor-not-allowed animate__animated animate__fadeIn"
        >
          Add to Cart
        </button>
      )}
